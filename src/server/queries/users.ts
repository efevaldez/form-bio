...
export const getUserByFile = async (file: string): Promise<user | null> => {
  ...
  select usr.file, usr.name, usr.site, sup.name as "supervisorName", sup.email as "supervisorEmail", array_agg( sru.name) as "siteResponsibleName", array_agg(sru.email) as "siteResponsibleEmail"
    from "Users" usr 
    join "Users" sup 
    ...
    where usr.file = ${file}
    group by usr.file, usr.name, usr.site, sup.name, sup.email`;
    console.log(result)
